...
function overlayWidth() {
  return lumine.config.get("scrollmap.overlayWidth") ?? 0;
}
...
class Scrollmap {
  constructor(editor, registry, filters) {
    ...
    const document = editor.getElement().ownerDocument;
    this.canvas = new toolkit.MarkerCanvas({ document, className: "scrollmap" });
  ...
  bindSurface() {
    ...
    this.resizeObserver = new domWindow.ResizeObserver(() => {
      this.canvas.invalidate();
      this.updateView();
    });
    this.resizeObserver.observe(this.editor.getElement());
    if (this.rafId != null && this.rafWindow !== domWindow) {
      this.cancelDrawFrame();
    }
    this.canvas.invalidate();
    this.drawSignature = null;
    // Commit runs while a detached renderer is hidden. Paint synchronously so
    // it is already correct when core shows the native Window.
    this.drawMarkers();
    this.updateView();
  }
  ...
  scheduleSurfaceFrame(callback) {
    if (this.destroyed) return;
    const domWindow = this.getWindow();
    const frame = { window: domWindow, handle: null };
    frame.handle = domWindow.requestAnimationFrame(() => {
      this.surfaceFrames.delete(frame);
      if (!this.destroyed && this.getWindow() === domWindow) callback();
    });
    this.surfaceFrames.add(frame);
  }

  cancelSurfaceFrames() {
    for (const frame of this.surfaceFrames) {
      try {
        frame.window.cancelAnimationFrame(frame.handle);
      } catch {
  ...
  updateTheme() {
    ...
    this.canvas.invalidate();
    // The styles moved, so the identical geometry must repaint anyway.
    this.drawSignature = null;
    this.drawMarkers();
  }
  ...
  drawMarkers() {
    ...
    // `did-update-decorations` fires on every cursor blink of a decorated
    // layer, so most calls arrive with nothing to show for it. The items
    // arrays stand in for their content -- the hub swaps one only when the
    // markers actually moved -- and every geometry input that could move a
    // pixel is listed alongside; block decorations are covered through
    // `getScrollHeight`, which their add, removal and resize all change.
    const signature = [
      editorHeight,
      clientHeight,
      lineHeight,
      width,
      this.getWindow().devicePixelRatio,
    ];
    for (const layer of drawn) {
      signature.push(layer.name, layer.items);
    }
    if (
      this.drawSignature &&
      this.drawSignature.length === signature.length &&
      this.drawSignature.every((value, i) => value === signature[i])
    ) {
      return;
    }
