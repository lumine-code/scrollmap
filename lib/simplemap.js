const { CompositeDisposable } = require("lumine");
const { coalesce } = require("./utils");
const toolkit = require("./toolkit");

let warnedHubless = false;

// The same strip, for a pane that is not a text editor.
//
// A PDF page or a notebook has no screen rows, so items arrive as percentages of
// the document and the caller sets them directly. Everything else -- the marker
// classes, the style probe, the z-index ordering -- is shared with the editor
// strip, so one stylesheet styles both.
class Simplemap {
  constructor(document, scrollbarWidth) {
    if (!document?.defaultView) {
      throw new TypeError("Simplemap requires a live Document");
    }
    this.items = [];
    this.styleSignature = null;
    this.document = document;
    this.setScrollbarWidth(scrollbarWidth);

    // The canvas machinery arrives through the marker hub. A widget consumer
    // constructs its strips long after service wiring, so the only way to land
    // here without it is an install with no `marker` package -- degrade to an
    // empty strip rather than crash the pane that asked for a widget.
    if (toolkit.MarkerCanvas) {
      this.canvas = new toolkit.MarkerCanvas({
        document,
        className: "simplemap",
        canvasClass: "scrollmap-canvas",
        resolverClass: "scrollmap-style-resolver",
        probeClass: "scrollmap-style-probe",
      });
      this.element = this.canvas.element;
    } else {
      if (!warnedHubless) {
        warnedHubless = true;
        console.warn("scrollmap: the marker package is not installed, widgets draw nothing");
      }
      this.canvas = null;
      this.element = document.createElement("div");
      this.element.className = "simplemap";
    }

    const updateTheme = coalesce(() => this.updateTheme());
    this.disposables = new CompositeDisposable(
      lumine.styles.onDidAddStyleElement(updateTheme),
      lumine.themes.onDidChangeActiveThemes(updateTheme),
    );
  }

  setItems(items) {
    this.items = items;
    this.drawMarkers();
  }

  setScrollbarWidth(width) {
    if (!Number.isFinite(width) || width < 0) {
      throw new TypeError("Simplemap requires a non-negative scrollbar width");
    }
    const value = `${width}px`;
    const root = this.document.documentElement;
    root.style.setProperty("--scrollbar-width", value);
    root.style.setProperty("--scrollbar-bottom", value);
  }

  update() {
    if (!this.canvas) {
      return;
    }
    this.canvas.invalidate();
    this.drawMarkers();
  }

  // Repaint a restyled window from within its cross-fade, and only when the
  // restyle reached the markers. See `Scrollmap.updateTheme`.
  updateTheme() {
    if (!this.canvas) {
      return;
    }
    const width = this.element.clientWidth;
    const height = this.element.clientHeight;
    const signature = this.canvas.signature(width, height);
    if (signature === this.styleSignature) {
      return;
    }
    this.styleSignature = signature;
    this.canvas.invalidate();
    this.drawMarkers();
  }

  destroy() {
    this.disposables.dispose();
    if (this.canvas) {
      this.canvas.destroy();
    } else {
      this.element.remove();
    }
  }

  drawMarkers() {
    if (!this.canvas || !this.element) {
      return;
    }

    const width = this.element.clientWidth;
    const height = this.element.clientHeight;
    if (width <= 0 || height <= 0) {
      return;
    }

    const regions = this.items.map((item) => {
      let className = "marker";
      if (item.position) {
        className += ` ${item.position}`;
      }
      if (item.cls) {
        className += ` ${item.cls}`;
      }
      let markerHeight = 0;
      if (item.end !== undefined) {
        markerHeight = ((item.end - item.prc) / 100) * height;
      } else if (item.height) {
        markerHeight = toolkit.resolveLength(item.height, height);
      }
      return {
        y: (item.prc / 100) * height,
        height: markerHeight,
        className,
      };
    });

    this.canvas.draw(regions, width, height);
  }
}

module.exports = Simplemap;
