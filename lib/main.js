...
module.exports = {
  activate() {
    ...
    this.simplemaps = new Set();
    this.SimplemapClass = null;
    this.measuredScrollbarWidth = null;
    this.picker = null;
    this.consumer = null;
    ...
    this.scrollbarWidth = lumine.config.get("scrollmap.overlayWidth") ?? 0;
    this.publishScrollbarWidth(document, this.scrollbarWidth);
  ...
  consumeMarkerRegistry(registry) {
    ...
    this.consumer = new CompositeDisposable(
      ...
      lumine.workspace.addWindowSurfaceTransitionObserver((context) => {
        const scrollmap = this.scrollmaps.get(context.item);
        if (!scrollmap) return null;
        scrollmap.prepareSurfaceTransition();
        const finish = () => {
          scrollmap.bindSurface();
          this.measureScrollbar(context.item);
        };
  ...
  attachEditor(editor, registry) {
    ...
    scrollmap.scheduleSurfaceFrame(() => {
      this.measureScrollbar(editor);
    });
  ...
  updateTheme() {
    // A UI theme can change the scrollbar width, and every renderer owns its
    // own root variables, so re-measure each surface before its layers read
    // their styles back.
    this.updateScrollbarGeometry();
    for (const editor of lumine.workspace.getTextEditors()) {
  ...
  updateScrollbarGeometry() {
    ...
    if (editors.length === 0) {
      this.setSharedScrollbarWidth(lumine.config.get("scrollmap.overlayWidth") ?? 0);
      this.publishScrollbarWidth(document, this.scrollbarWidth);
      return;
    }
  ...
  setSharedScrollbarWidth(width) {
    if (width === this.scrollbarWidth) return;
    this.scrollbarWidth = width;
    for (const simplemap of this.simplemaps) {
      simplemap.setScrollbarWidth(width);
    }
  },

  publishScrollbarWidth(document, width) {
    const value = `${width}px`;
    const root = document.documentElement;
    if (
      root.style.getPropertyValue("--scrollbar-width") !== value ||
      root.style.getPropertyValue("--scrollbar-bottom") !== value
    ) {
  ...
  measureScrollbar(editor) {
    ...
    const measuredWidth = editor?.getElement()?.component?.getVerticalScrollbarWidth();
    if (measuredWidth > 0) {
      this.measuredScrollbarWidth = measuredWidth;
    }
    const width =
      measuredWidth ||
      this.measuredScrollbarWidth ||
      lumine.config.get("scrollmap.overlayWidth") ||
      0;
    this.setSharedScrollbarWidth(width);
    const surfaceDocument = editor?.getElement()?.ownerDocument || document;
    this.publishScrollbarWidth(surfaceDocument, width);
  },

  provideScrollmapWidget() {
    if (this.SimplemapClass) return this.SimplemapClass;
    const Simplemap = require("./simplemap");
    const main = this;
    this.SimplemapClass = class ScrollmapWidget extends Simplemap {
      constructor(document) {
        super(document, main.scrollbarWidth);
        main.simplemaps.add(this);
      }

      destroy() {
        main.simplemaps.delete(this);
        super.destroy();
      }
    };
    return this.SimplemapClass;
  },
};
