...
// A digest of everything `drawMarkerRegions` reads off the style probes the last
// draw left behind. Comparing two digests answers the only question a stylesheet
// change raises: would drawing again put different pixels on the canvas?
function getStyleSignature(element) {
  const probes = getResolver(element)?._scrollmapStyleProbes;
  if (!probes?.size) {
    return "";
  }
  ...
  const parts = [];
  for (const className of probes.keys()) {
    const style = getMarkerStyle(element, className, width, height);
    parts.push(`${className}:${style ? Object.values(style).join(",") : ""}`);
  }
  return parts.join("|");
}
